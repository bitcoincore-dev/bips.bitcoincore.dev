...
This work is licensed under a <a href="http://creativecommons.org/licenses/by/4.0/" target="_blank">Creative Commons Attribution 4.0 International License</a>.
...
The motivation for defining this extension to the <a href="/70" target="_blank">BIP70</a> Payment Protocol is to allow two parties to exchange payment information in a permissioned and encrypted way, such that wallet address communication can become a more automated process. This extension also expands the types of PKI (public-key infrastructure) data that is supported, and allows it to be shared by both parties (with <a href="/70" target="_blank">BIP70</a>, only the receiver could provide PKI information). This allows for automated creation of off-blockchain transaction logs that are human readable, now including information about the sender and not just the recipient.

The motivation for this extension to <a href="/70" target="_blank">BIP70</a> is threefold:
...
A Bitcoin wallet developer would like to allow users to view a potential sending party's identifying information before deciding whether or not to share payment information with them. Currently, <a href="/70" target="_blank">BIP70</a> shares the receiver’s payment address and identity information with anyone who requests it.
...
<h2>Modifying BIP70 pki_type</h2>
...
Updated <a href="/bip-0075/paymentrequest.proto" target="_blank">paymentrequest.proto</a> contains the existing PaymentRequest Protocol Buffer messages as well as the messages newly defined in this BIP.
...
1.  Sender encapsulates InvoiceRequest in (Encrypted)ProtocolMessage
1.  Sender sends (Encrypted)ProtocolMessage to Receiver
1.  Receiver retrieves InvoiceRequest in (Encrypted)ProtocolMessage from Sender
1.  Receiver creates PaymentRequest
1.  Receiver encapsulates PaymentRequest in EncryptedProtocolMessage
1.  Receiver transmits EncryptedProtocolMessage to Sender
1.  Sender validates PaymentRequest retrieved from the EncryptedProtocolMessage
...
<h2>Message Interaction Details</h2>


<h3>HTTP Content Types for New Message Types</h3>
...
<h4>Payment Protocol Status Codes</h4>
...
This BIP extends the Payment Protocol as defined in <a href="/70" target="_blank">BIP70</a>.
...
<h3>InvoiceRequest Validation</h3>
...
<h3>ECDH Point Generation and AES-256 (GCM Mode) Setup</h3>
...
*  Generate the **secret point** using <a href="https://en.wikipedia.org/wiki/Elliptic_curve_Diffie–Hellman" target="_blank">ECDH</a> using the local entity's private key and the remote entity's public key as inputs
*  Initialize <a href="http://csrc.nist.gov/publications/nistpubs/800-90A/SP800-90A.pdf" target="_blank">HMAC_DRBG</a>
...
<h4>AES-256 GCM Authentication Tag Use</h4>
...
<h4>AES-256 GCM Additional Authenticated Data</h4>
...
1.  Address Service Public Key Exposure


<h2>Payment / PaymentACK Messages with a HTTP Store & Forward Server</h2>
...
<h2>Public Key & Signature Encoding</h2>

*  All x.509 certificates included in any message defined in this BIP MUST be DER [ITU.X690.1994] encoded.
*  All EC public keys (**sender_public_key**, **receiver_public_key**) in any message defined in this BIP MUST be <a href="SECP256k1" target="_blank">http://www.secg.org/sec2-v2.pdf</a> ECDSA Public Key ECPoints encoded using <a href="SEC 2.3.3 Encoding" target="_blank">http://www.secg.org/sec1-v2.pdf</a>. Encoding MAY be compressed.
...
*  All OpenPGP certificates must follow <a href="https://tools.ietf.org/html/rfc4880" target="_blank">RFC4880</a>, sections 5.5 and 12.1.
...
<a href="https://github.com/netkicorp/addressimo" target="_blank">Addressimo</a>

A reference client implementation can be found in the InvoiceRequest functional testing for Addressimo here:

<a href="https://github.com/netkicorp/addressimo/blob/master/functest/functest_bip75.py" target="_blank">BIP75 Client Reference Implementation</a>

<h2>BIP70 Extension</h2>

The following flowchart is borrowed from <a href="/70" target="_blank">BIP70</a> and expanded upon in order to visually describe how this BIP is an extension to <a href="/70" target="_blank">BIP70</a>.

<img src="bip-0075/bip70-extension.png" alt="Flowchart explaining how this BIP extends BIP 70">

<h2>Mobile to Mobile Examples</h2>
...
*  <a href="/70" target="_blank">BIP70 - Payment Protocol</a>
*  <a href="https://en.wikipedia.org/wiki/Elliptic_curve_Diffie–Hellman" target="_blank">ECDH</a>
*  <a href="http://csrc.nist.gov/publications/nistpubs/800-90A/SP800-90A.pdf" target="_blank">HMAC_DRBG</a>
*  <a href="http://csrc.nist.gov/publications/nistpubs/800-38D/SP-800-38D.pdf" target="_blank">NIST Special Publication 800-38D - Recommendation for Block Cipher Modes of Operation: Galois/Counter Mode (GCM) and GMAC</a>
*  <a href="https://tools.ietf.org/html/rfc6979" target="_blank">RFC6979</a>
*  <a href="https://en.bitcoin.it/wiki/Address_reuse" target="_blank">Address Reuse</a>
*  <a href="http://csrc.nist.gov/publications/fips/fips180-4/fips-180-4.pdf" target="_blank">FIPS 180-4 (Secure Hash Standard)</a>
