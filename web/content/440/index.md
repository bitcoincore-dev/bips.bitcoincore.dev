...
bip = 440
status = ["Draft"]
github = "https://github.com/bitcoin/bips/blob/master/bip-0440.mediawiki"
...
This BIP defines a "varops budget", which generalizes the "sigops budget" introduced in <a href="/342" target="_blank">BIP342</a> to non-signature operations.
...
<h3>A Model For Opcodes Dealing With Stack Data</h3>
...
1.  the script can only consume the varops budget of a whole block: 10,000 * 4,000,000 (40b).
...
The signature cost is simply carried across from the existing <a href="/342" target="_blank">BIP-342</a> limit: 50 weight units allow you one signature.  Since each transaction gets varops budget for the entire transaction (not just the current input's witness), and each input has at least 41 bytes (164 weight), this is actually slightly more generous than the sigops budget (which was 50 + witness weight), but still limits the entire block to 80,000 signatures.
...
1.  OP_ROLL, which does a large-scale stack movement.
1.  Fast operations: comparing bytes, comparing bytes against zero, and zeroing bytes.  Empirically, these have been shown to be well-optimized.
1.  Copying bytes: slightly more expensive than fast operations due to memory allocation overhead.
1.  Everything else.
...
1.  Hashing costs 50 units per byte hashed.
1.  OP_ROLL costs an additional 48 units (24 bytes per std::vector * 2 units per byte) per stack entry moved (i.e. the value of its operand).
1.  Fast operations cost 2 units per byte output.
1.  Copying operations cost 3 units per byte output.
1.  Arithmetic operations (which don't pipeline as well due to the overflow between words) cost 6 units per byte output.
1.  Other operations cost 4 units per byte output.


<h3>Variable Opcode Budget</h3>
...
<h3>Example: Stack Manipulation</h3>
...
<h3>Example: Comparison Operators</h3>
