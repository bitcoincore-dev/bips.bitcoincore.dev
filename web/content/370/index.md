...
The PSBT version number is set to 2. The Creator should also set
...
PSBT_GLOBAL_TX_MODIFIABLE field must be present and the Inputs
Modifiable and Outputs Modifiable flags set appropriately; moreover, the
transaction version number must be set to at least 2. [^3] If the
...
- Case: 1 input, 2 output updated PSBTv2, with an undefined flag (bit 3)
...
- Case: Input 1 has PSBT_IN_REQUIRED_TIME_LOCKTIME of 1657048459, Input
  2 has PSBT_IN_REQUIRED_HEIGHT_LOCKTIME of 9000 and
  PSBT_IN_REQUIRED_TIME_LOCKTIME of 1657048460
  - Bytes in Hex:
