...
- While the relayed data itself is public in nature, the associated
  ...
  transaction.
- Since connections are unauthenticated, they can be tampered with at a
  ...
  on-the-fly without the need to keep any state.
- The protocol is self-revealing. For example, deep packet inspection
  ...
  of victims, e.g., eclipse attacks targeted at miners.
...
- Tampering, while already an inherently active attack, is costlier if
  ...
  interception.
- A pseudorandom bytestream excludes identification techniques based on
...
1.  The **Version negotiation phase**, where parties negotiate what
    ...
    - The responder:
      - Sends a **version packet** with empty content, to indicate
        ...
        other value for content is reserved for future versions.
    - The initiator:
...
- *XSwiftEC(u, t)*:
  - Alter the inputs to guarantee an X coordinate on the curve:[^16]
    ...
    - If *(u<sup>3</sup> + t<sup>2</sup> + 7) mod p = 0*, let *t = 2t
      (mod p)* instead.
  - Let *X = (u<sup>3</sup> + 7 - t<sup>2</sup>)/(2t) (mod p).*[^17]
  - Let *Y = (X + t)/(cu) (mod p)*.
  - For every *x* in *{u + 4Y<sup>2</sup>, (-X/Y - u)/2, (X/Y - u)/2}*
    (all *mod p*; the order matters):
    - If *lift_x(x)* succeeds, return *x*. There is at least one such
      *x*.
...
paper, then uses this inverse to randomly*[^19] sample encodings of*x'':
...
- **ChaCha20Poly1305** is specified as `AEAD_CHACHA20_POLY1305` in [RFC
  8439 section
  2.8](https://datatracker.ietf.org/doc/html/rfc8439#section-2.8). It is
  ...
  - `aead_chacha20_poly1305_encrypt(key, nonce, aad, plaintext)` refers
    to a function that takes as input a 32-byte array *key*, a 12-byte
    array *nonce*, an arbitrary-length byte array *aad*, and an
    arbitrary-length byte array *plaintext*, and returns a byte array
    *ciphertext*, 16 bytes longer than the plaintext.
  - `aead_chacha20_poly1305_decrypt(key, nonce, aad, ciphertext)` refers
    to a function that takes as input a 32-byte array *key*, a 12-byte
    array *nonce*, an arbitrary-length byte array *aad*, and an
    arbitrary-length byte array *ciphertext*, and returns either a byte
    array *plaintext* (16 bytes shorter than the ciphertext), or *None*
...
- The **ChaCha20 Block Function** is specified in [RFC 8439 section
  ...
  - `chacha20_block(key, nonce, count)` refers to a function that takes
    as input a 32-byte array *key*, a 12-byte array *nonce*, and an
    integer *count* in range *0..2<sup>32</sup>-1*, and returns a byte
    array of length 64.
...
[^1]: **What does *authentication* mean in this context?**
    ...
    - The encryption scheme guaranteeing that a message obtained via
      ...
      the (symmetric) encryption key, and not modified after encryption
      by a third party. The proposal in this document achieves that
      property through the use of an AEAD.
    ...
    *session ID* that uniquely identifies the encrypted channel. After
    ...
    - Observability of active attacks: A session ID identifying the
      ...
      is forced to incur a risk of being detected as peer operators can
...
[^6]: **How can progress be guaranteed in a backwards-compatible way?**
    ...
    - As soon as either party has received the other peer's garbage
      ...
      met, the other party has nothing to respond with anymore that
      would be needed to guarantee progress otherwise.)
...
[^7]: **Is *2<sup>24</sup>-1* bytes sufficient as maximum content
    size?** The current Bitcoin P2P protocol has no messages which
    support more than 4000000 bytes of application payload. By
    supporting up to *2<sup>24</sup>-1* we can accommodate future
    ...
    the length field, if this were really needed.
...
[^11]: **How does packet encryption differ from the OpenSSH design?**
    ...
    - The Poly1305 authentication tag only covers the encrypted
      ...
      be seen as a separate layer, using a separate key, and thus cannot
      affect any of the confidentiality or integrity guarantees of the
      plaintext encryption. On the other hand, this change w.r.t.
      OpenSSH also does not worsen any properties, as incorrect lengths
...
[^16]: **Why do the inputs to the XSwiftEC algorithm need to be
    ...
    negligibly small subset of inputs (around *3/2<sup>256</sup>*) to
    ...
    case, we remap them to inputs that yield a valid X coordinate.
